...
class OrderSummaryController extends GetxController {
  ...
  fetchOrderSummary() async {
    orderSummaryResponse = await Get.find<CheckoutRepository>().fetchOrderSummary();
  }

  placeOrder(BuildContext context, ConfirmOrderParams confirmOrderParams) async {
    showLoadingDialog(context);
    String? shippingAddressId = confirmOrderParams.shippingAddress?.id.toString();
    String? billingAddressId = confirmOrderParams.billingAddress?.id.toString();
    final result =
        await Get.find<CheckoutRepository>().placeOrder(billingAddressId!, shippingAddressId!);
