...
class HomeBannerRepositoryImpl extends HomeBannerRepository {
  ...
  HomeBannerRepositoryImpl({required this.homeBannerRemoteDataSource, required this.networkInfo});

  @override
  Future<ApiResponse> getBannerList(String identifier) async {
    if (await networkInfo.isConnected) {
      try {
        final response = await homeBannerRemoteDataSource.getBannerList(identifier);
        final mappedHomeBanner =
            response[0]['image'].map((e) => HomeBannerResponse.fromJson(e)).toList();
